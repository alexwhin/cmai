...
export class ConfigurationNotFoundError extends ConfigurationError {
  constructor(path?: string) {
    const message = path
      ? t("errors.configuration.notFound", { path })
      : t("errors.configuration.notFoundNoPath");
...
export class ConnectionRefusedError extends NetworkError {
  constructor(provider?: string) {
    const message = provider
      ? t("errors.network.connectionRefusedWithProvider", { provider })
      : t("errors.network.connectionRefusedNoProvider");
    super(message);
  }
}
...
export class ServerNotFoundError extends NetworkError {
  constructor(provider?: string) {
    const message = provider
      ? t("errors.network.serverNotFoundWithProvider", { provider })
      : t("errors.network.serverNotFoundNoProvider");
    super(message);
  }
}
...
function getErrorMessage(error: BaseError | Error): string {
  ...
  return error instanceof Error
    ? error.message
    : t("errors.unknown", { message: "An unexpected error occurred" });
}
  ...
  if (errorInstance instanceof Error && errorInstance.message.includes("API error")) {
    ...
    const errorMessage =
      errorInstance instanceof Error
        ...
        : t("errors.unknown", { message: "Unknown error" });
